...
`);
    ...
    dclient.on('message', message => {
        ...
        /*if (message.content.startsWith(prefix + 'hypixel1')) {
		var suffix = message.content.split(" ").slice(1).join(" ");
		var gamemode = message.content.split(" ").slice(2).join(" ");
		if (suffix == "" || suffix == null) return message.reply("Please enter a Minecraft usernameto query.");
        if (gamemode == "" && message.content.split(" ").slice(2).toLowerCase().match(/bedwars|megawalls|skywars/gmi) !== null) {
        //if (suffix == "" || suffix == null) return message.channel.sendMessage("Do " + config.prefix + "hypixel <username?> for Hypixel Status!");
			request("https://api.hypixel.net/player?key=d8353545-a9e6-418c-8cfd-9c21bee7370e&name=" + suffix, function(error, response, body) { //set info for the streamer in JSON
				if (error) {
					console.log('Error encounterd: ' + err);
					message.channel.send("Horrible stuff happend D:. Try again later.");
					return;
				...
				console.log(suffix)
				var json = JSON.parse(body);
				if (message.content.split(" ").slice(2).toLowerCase().match(/bedwars|megawalls|skywars/gmi)[0] !== "bedwars") {
					if (!error && response.player == null) {
						if (json.player !== null) {				
							var lastLogin = new Date(json.player.lastLogin);
							var firstLogin = new Date(json.player.firstLogin);
				...
				if (message.content.split(" ")[2].toLowerCase().match(/bedwars|megawalls|skywars/gmi)[0] !== "megawalls") {
					
				}else
				if (message.content.split(" ")[2].toLowerCase().match(/bedwars|megawalls|skywars/gmi)[0] !== "skywars") {
